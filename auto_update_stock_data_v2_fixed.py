...
class TushareAPIFixed:
    """Tushare API 修复版本 - 解决权限问题"""
    ...
    def __init__(self, token="46813b22149a178b86d12174cf301142e68fe13445129dd34a8cbcad"):
        ...
        token_file = os.path.join(project_dir, 'tushare_token.csv')
        ...
        # 创建token文件在项目目录
        try:
            token_df = pd.DataFrame({'token': [token]})
            token_df.to_csv(token_file, index=False)
            
            # 设置环境变量，强制使用项目目录的token文件
...
class AutoStockDataUpdaterV2Fixed:
    """A股数据自动更新器 - 版本2.0 修复版"""
...
def main():
    ...
    # 创建自动更新器
    updater = AutoStockDataUpdaterV2Fixed(data_dir=DATA_DIR, config_file=CONFIG_FILE)
