...
# -*- coding: utf-8 -*-
"""
A股数据自动更新工具 - 版本2.0 IP直连修复版

修复问题：
1. 使用IP地址直连，绕过DNS解析问题
2. 支持完整5395只股票更新
3. 智能重试机制
...
class TushareAPIIP:
    """Tushare API IP直连版本 - 绕过DNS解析"""
    
    def __init__(self, token="46813b22149a178b86d12174cf301142e68fe13445129dd34a8cbcad"):
        """初始化Tushare API IP直连版本"""
        self.token = token
        
        # 使用IP地址直连，绕过DNS解析
        self.base_urls = [
            "http://116.62.129.122",  # Tushare API IP地址1
            "http://47.107.33.27",    # Tushare API IP地址2
            "http://api.tushare.pro"  # 域名（备用）
        ]
        
        self.current_url_index = 0
        
        print("Tushare API IP直连版初始化成功")
...
class AutoStockDataUpdaterV2IP:
    """A股数据自动更新器 - IP直连版本"""
...
def main():
    """主函数"""
    print("A股数据自动更新工具 - 版本2.0 IP直连修复版")
    print("=" * 50)
    ...
    # 创建自动更新器
    updater = AutoStockDataUpdaterV2IP(data_dir=DATA_DIR, config_file=CONFIG_FILE)
