...
# -*- coding: utf-8 -*-
"""
A股数据自动更新工具 - 版本2.0 智能版

修复问题：
1. 智能请求频率控制，避免HTTP 405错误
2. 自适应重试机制，处理服务器超时
3. 批量下载优化，提高成功率
...
class TushareAPISmart:
    """Tushare API 智能版本 - 带频率控制和智能重试"""
    ...
    def __init__(self, token="46813b22149a178b86d12174cf301142e68fe13445129dd34a8cbcad"):
        ...
        print("Tushare API 智能版初始化成功")
        print(f"使用Token: {token[:10]}...")
        print("频率控制: 最小间隔", self.min_interval, "秒")
    ...
    def _make_request(self, api_name, params):
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                # 动态调整超时时间
                timeout = 10 + attempt * 5  # 每次重试增加5秒超时
                ...
                if response.status_code == 200:
                    ...
                    print(f"  HTTP 405错误 (尝试 {attempt + 1}/{self.max_retries})")
...
class AutoStockDataUpdaterV2Smart:
    """A股数据自动更新器 - 智能版本"""
    ...
    def initial_download_all_markets(self, full_download=False):
        ...
        if not full_download:
            stock_basic = stock_basic.head(10)  # 测试模式只下载10只股票
            print("测试模式: 只下载前10只股票")
        ...
        if all_data:
            ...
            if is_update and os.path.exists(file_path):
                ...
                try:
                    ...
                    print(f"✅ {market_info['name']}数据更新成功: 新增{len(combined_data)}条记录, 总记录{len(merged_data)}条")
    ...
    def update_market_data(self, market):
        ...
        try:
            ...
            # 获取最新数据（从最后一条记录的下一天开始）
            last_date = existing_data['trade_date'].max()
            ...
            self._download_market_data(market, stock_codes, start_date, end_date, is_update=True)
...
def main():
    ...
    # 创建自动更新器
    updater = AutoStockDataUpdaterV2Smart(data_dir=DATA_DIR, config_file=CONFIG_FILE)
